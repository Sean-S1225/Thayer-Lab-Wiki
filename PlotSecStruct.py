...
for subfig, file, title in zip(subfigs, ["12A_001_v1", "12A_001_v2", "15A_001_v1", "15A_001_v2", "15A_0005", "15A_002", "15A_00225"],
    ...
    wt = pd.read_csv(f"../Data/SecStruct/{file}_WT_SecStructTime.dat", sep="\s+").drop("#Frame", axis=1)[:1000]
    y220c = pd.read_csv(f"../Data/SecStruct/{file}_Y220C_SecStructTime.dat", sep="\s+").drop("#Frame", axis=1)[:1000]
    pk11000 = pd.read_csv(f"../Data/SecStruct/{file}_PK11000_SecStructTime.dat", sep="\s+").drop("#Frame", axis=1)[:1000]
    ...
    cbar = plt.colorbar(im, ax = axs[:-1], location="bottom")
...
fig.savefig("SecStructMatrix.png", bbox_inches = "tight", dpi = 500)